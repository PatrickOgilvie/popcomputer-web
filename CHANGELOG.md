...
- **`findRelation` is now async and returns JS property keys**: It dynamically imports `drizzle-orm` and resolves `{ foreignKey, references }` as JS property keys (`workspaceId`) rather than SQL column names (`workspace_id`). **Breaking** for code that called `findRelation` directly.
